...
def post(url, data=None, cookie=None, header=None, is_json=False, files=None, params=None):
    ...
    try:
        ...
        # log.debug("【耗时：%s。接口返回数据：%s】" % (post_time_used, res))
...
def get(url, data=None, cookie=None, header=None):
    ...
    # log.debug("【接口请求数据：%s】" % data)
    # print("【请求接口地址：%s】" % url)
    # print("【接口请求数据：%s】" % data)
    try:
        get_start_time = time.time()
        res = requests.get(url, params=data, cookies=cookie, headers=header, verify=False, timeout=10).json()
        # get_end_time = time.time()
        # get_time_used = get_end_time - get_start_time
        # log.debug("【耗时：%s。接口返回数据：%s】" % (get_time_used, res))
